...
function RaceList({ season, onRaceSelect }) {
    ...
    Promise.all([
      ...
      .then(([sessionsResponse, meetingsResponse]) => {
        ...
        if (!Array.isArray(raceSessions) || !Array.isArray(meetingsData)) {
          console.error(
            "Unexpected data format from OpenF1 API. Expected arrays.",
            { sessions: raceSessions, meetings: meetingsData }
          );
        ...
        let filteredSessions = raceSessions.filter(
      ...
      .catch((err) => {
        console.error(
          `Error fetching race data for season ${season} from OpenF1:`,
          err
        );
        if (err.response) {
          setError(
            `Failed to load races for ${season}. OpenF1 API responded with status ${err.response.status}.`
          );
        } else if (err.request) {
          setError(
            `Failed to load races for ${season}. No response from OpenF1 API. Check network.`
          );
        } else {
          setError(
            `Failed to load races for ${season}. Error setting up request to OpenF1 API.`
          );
        }
        setRaces([]);
        setIsLoading(false);
  ...
  if (isLoading || isLoadingCircuits) {
    // Check both loading states
    return (
      <p>
        Loading data for {season} from OpenF1... (Races:{" "}
        {isLoading ? "Loading..." : "Done"}, Circuits:{" "}
        {isLoadingCircuits ? "Loading..." : "Done"})
      </p>
    );
  }

  if (error) {
    // Main error for races
    return <p style={{ color: "red" }}>{error}</p>;
  }
  if (circuitsError) {
    // Display circuit specific error (could be less critical)
    // Decide if this should block rendering races or just be a warning
    return (
      <p style={{ color: "orange" }}>
        Warning: {circuitsError} Races may display without full circuit details.
      </p>
    );
  }

  if (races.length === 0 && !isLoading) {
    // Ensure not to show "no races" if still loading
    return (
      <p>
        No races found for the {season} season using OpenF1. The API might have
        no data for this year or there was an issue.
      </p>
    );
  }
  ...
  return (
    <div className="race-list-section" style={cardStyle}>
      ...
      <div className="race-list-grid-container">
        {races.map((race) => {
          ...
          const circuitDisplayName =
            circuitInfo?.short_name || race.circuit_short_name;
          ...
          const altText = circuitInfo
            ? `Circuit layout for ${circuitInfo.name}`
            : `Circuit layout for ${race.circuit_short_name}`;
            ...
            <div key={race.session_key} className="race-list-item-card">
              <img
                ...
                onError={(e) => {
                  ...
                  if (
                    placeholder &&
                    placeholder.classList.contains("circuit-image-placeholder")
                  ) {
              ...
              <div className="race-card-details">
                <h4 className="race-card-name">
                  Round {race.round}: {race.meeting_name}
                </h4>
                ...
                <p className="race-card-laps">
                  Laps: {race.total_laps || "N/A"}
                </p>
              </div>

              <button
                onClick={() => handleRaceClick(race)}
                className="race-card-button"
              >
                View Race Details
              </button>
            </div>
