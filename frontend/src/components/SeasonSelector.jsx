...
function SeasonSelector({ onSeasonSelect }) {
  ...
  useEffect(() => {
    ...
    setError("");
    axios
      .get(`${OPENF1_BASE_URL}/meetings?year>=2018`)
      .then((response) => {
        if (response.data && Array.isArray(response.data)) {
          const years = [
            ...new Set(response.data.map((meeting) => meeting.year)),
          ].sort((a, b) => b - a);
          setSeasons(years);
        } else {
          setSeasons([]);
      ...
      .catch((err) => {
        ...
        if (err.response) {
          setError(
            `Failed to load seasons. API responded with status ${err.response.status}.`
          );
        } else if (err.request) {
          setError(
            "Failed to load seasons. No response from OpenF1 API. Check network."
          );
        } else {
          setError(
            "Failed to load seasons. Error setting up request to OpenF1 API."
          );
        }
        setSeasons([]);
        setIsLoading(false);
  ...
  return (
    <div style={selectorStyle}>
      ...
      {seasons.length === 0 && !isLoading && !error && (
        <p>
          No seasons loaded. The OpenF1 API might be down or returned no data.
        </p>
      )}
    </div>
  );
