...
const baseChartOptions = {
  ...
  scales: {
    x: {
      title: { display: true, text: 'Lap Number', color: 'var(--color-text-secondary)' },
      ticks: { color: 'var(--color-text-secondary)' },
      grid: { color: 'var(--color-border)' }
    },
    y: {
      title: { display: true, text: 'Gap (seconds)', color: 'var(--color-text-secondary)' },
      ticks: {
        color: 'var(--color-text-secondary)',
        callback: function(value) { return formatGapTime(value, false); }
      },
      grid: { color: 'var(--color-border)' }
    }
  },
  plugins: {
    ...
    title: { display: true, text: 'Driver Gap Over Race Laps', color: 'var(--color-text-primary)', font: { size: 16 }},
    tooltip: {
      callbacks: {
        label: function(context) {
          let label = context.dataset.label || '';
          if (label) { label += ': '; }
          if (context.parsed.y !== null) { label += formatGapTime(context.parsed.y, true); }
          return label;
        }
      }
    }
...
function DriverGapChart({ selectedRace, allSessionDrivers }) {
  ...
  useEffect(() => {
    ...
    if (driver1 && driver2 && sessionLapsData && sessionLapsData.length > 0 && allSessionDrivers && allSessionDrivers.length > 0) {
      ...
      try {
        ...
        const commonLaps = [];
        const overallMaxLap = sessionLapsData.reduce((max, lap) => Math.max(max, lap.lap_number || 0), 0);
        ...
        if (driver1Info && driver2Info && driver1Info.team_name === driver2Info.team_name) {
          borderDashStyle = [5, 5];
        }
          ...
          datasets: [
            {
              ...
              tension: 0.2,
              fill: false,
              pointRadius: Math.max(1, Math.min(3, Math.floor(150 / commonLaps.length))),
              pointHoverRadius: 5,
      ...
      setChartData(null);
      if (driver1 && driver2 && (!allSessionDrivers || allSessionDrivers.length === 0)) {
    ...
    <div className="driver-gap-selectors">
      <div className="driver-selector">
        ...
        <select
          id="driver1-select"
          value={driver1 || ''}
          onChange={(e) => { setDriver1(e.target.value ? parseInt(e.target.value) : null); setChartData(null); }}
          disabled={!allSessionDrivers || allSessionDrivers.length === 0 || isLoadingLaps || isProcessingChart}
        >
      ...
      <div className="driver-selector">
        ...
        <select
          id="driver2-select"
          value={driver2 || ''}
          onChange={(e) => { setDriver2(e.target.value ? parseInt(e.target.value) : null); setChartData(null);}}
          disabled={!allSessionDrivers || allSessionDrivers.length === 0 || isLoadingLaps || isProcessingChart}
        >
  ...
  const currentChartOptions = {
    ...
    plugins: {
      ...
      title: {
        ...
        text: driver1 && driver2 && allSessionDrivers && allSessionDrivers.length > 0 ?
              `Gap: ${allSessionDrivers.find(d=>d.driver_number === driver2)?.name_acronym || 'D2'} to ${allSessionDrivers.find(d=>d.driver_number === driver1)?.name_acronym || 'D1'}` :
  ...
  return (
    <div className="driver-gap-chart-container">
      ...
      {(!allSessionDrivers || allSessionDrivers.length === 0) && !isLoadingLaps && (
          <p className="chart-placeholder">Driver list not available, cannot render selectors.</p>
      )}
      {allSessionDrivers && allSessionDrivers.length > 0 && renderDriverSelectors()}

      {isLoadingLaps && <div className="loading-message"><p>Loading session lap data...</p></div>}
      {isProcessingChart && !isLoadingLaps && <div className="loading-message"><p>Processing chart data...</p></div>}
      {error && !isLoadingLaps && !isProcessingChart && <div className="error-message"><p>{error}</p></div>}

      {!isLoadingLaps && !isProcessingChart && !error && (!sessionLapsData || sessionLapsData.length === 0) && (
        <div className="chart-placeholder">
          <p>No lap data available for this session.</p>
        </div>
      )}

      {!isLoadingLaps && !isProcessingChart && !error && sessionLapsData && sessionLapsData.length > 0 && (!driver1 || !driver2) && allSessionDrivers && allSessionDrivers.length > 0 && (
        <div className="chart-placeholder">
          <p>Lap data loaded. Select two drivers to compare their lap-by-lap gap.</p>
        </div>
      )}

      {!isLoadingLaps && !isProcessingChart && !error && chartData && driver1 && driver2 && (
        <div className="driver-gap-chart-area">
           <Line data={chartData} options={currentChartOptions} />
        </div>
      )}

      {!isLoadingLaps && !isProcessingChart && !error && sessionLapsData && driver1 && driver2 && !chartData && (
         <div className="chart-placeholder">
            <p>Chart will appear here. If not, selected drivers might have no common laps or an error occurred during processing.</p>
         </div>
      )}
    </div>
  );
