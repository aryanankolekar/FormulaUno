...
const TireStrategyGraph = ({ raceData, driverData }) => {
  ...
  const sortedDrivers = [...driverData].sort((a, b) => {
    // Sort by finishing position (race classification)
  ...
  const lapTicks = Array.from(
    { length: Math.floor(totalLaps / lapTickInterval) + 1 },
    (_, i) => i * lapTickInterval
  );
  if (
    lapTicks[lapTicks.length - 1] < totalLaps &&
    totalLaps % lapTickInterval !== 0
  ) {
  ...
  return (
    ...
    >
      ...
      <svg
        width={svgWidth}
        height={svgHeight}
        viewBox={`0 0 ${graphWidth + margin.left + margin.right} ${svgHeight}`}
      >
        <g transform={`translate(${margin.left}, ${margin.top})`}>
          ...
          {lapTicks.map((lap) => (
            <g
              key={`lap-tick-${lap}`}
              transform={`translate(${lapScale(lap)}, ${graphHeight})`}
            >
          ...
          {sortedDrivers.map((driver, index) => {
            const driverYPosition =
              (index / sortedDrivers.length) * graphHeight +
              graphHeight / sortedDrivers.length / 2;
            ...
            return (
              <g key={`driver-row-${driver.id || driver.driver_number}`}>
                ...
                >
                  {`P${driver.finishingPosition || "?"} ${
                    driver.name_acronym || driver.name
                  }`}
                </text>
                ...
                {/* Tire Stints */}
                {driver.stints &&
                  driver.stints.map((stint, stintIndex) => {
                    const startLap = stint.lapStart || 0; // This is mapped from lap_start in API
                    let endLap = stint.lapEnd || totalLaps; // This is mapped from lap_end in API

                    // Validate lap numbers
                    if (startLap === 0 && endLap === 0) {
                      console.warn(
                        `Driver ${driver.name} stint ${stintIndex} has invalid lap numbers:`,
                        stint
                      );
                      return null;
                    }
                    ...
                    const compoundUpper = stint.compound
                      ? stint.compound.toUpperCase()
                      : "UNKNOWN";
                    const color =
                      TIRE_COLORS[compoundUpper] || TIRE_COLORS.UNKNOWN;

                    if (x1 > x2) {
                      // Should not happen with correct data
                      console.warn(
                        "Stint start lap is after end lap:",
                        stint,
                        "for driver",
                        driver.name
                      );
